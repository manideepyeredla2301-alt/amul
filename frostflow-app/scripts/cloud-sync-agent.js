...
function businessRows(db){
 ...
 if(hasTable(db,'amul_routes'))for(const r of db.prepare('SELECT * FROM amul_routes WHERE local_deleted=0').all())data.routes.push({id:'AMUL:'+r.route_id,source:'AMUL',source_id:String(r.route_id),code:r.route_code||'',name:r.route_name,active:!!r.active,source_updated_at:r.synced_at||''});
